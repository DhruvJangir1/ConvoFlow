...
Client (Browser / React)
       │
       ▼
  Vite Proxy (dev)  ──►  server.js  ──►  auth.ts (router hub)
                                                    │
                          ┌─────────────────────────┼─────────────────────────┐
                          │                         │                         │
                          ▼                         ▼                         ▼
              AuthEmailVerification    AuthTokenVerification      AuthUserVerification
              ──────────────────────   ──────────────────────     ─────────────────────
              /signup                  /session                    /verify
              /login                   /refresh                    /resend-verification
              /logout
              /check-password
                          │                         │                         │
                          ▼                         ▼                         ▼
                    services/                  services/                 services/
                    auth.ts                    auth.ts                   verificationStore.ts
                    rateLimiter.ts             auth.ts                   authVerificaiton.ts
                    authVerificaiton.ts        ──────────────────        auth.ts
                    ──────────────────         Token validation          ──────────────────
                    Email/password             & refresh                  Code verification
...
app.use("/api/chats", ChatRouter);   // All chat-related routes
app.get("/api/health", ...);         // Health check (no router)
```

> **Key point:** `server.js` never defines route handlers directly (except `/health`). It only mounts routers. This keeps the file thin and focused on server configuration.
...
server.js
  │
  ├── Global middleware (JSON, CORS, cookies, CSRF)
  │
  ├── /api/auth ──► auth.ts
...
- **`server.js`** — Server config, middleware stack, router mounting. Never defines route handlers.
- **`auth.ts`** — Router hub. Only mounts sub-routers. Never defines route handlers.
- **Sub-routers** — Define route handlers for a specific domain (email auth, token auth, user verification).
- **Services** — Pure business logic (password hashing, JWT signing, rate-limiting). No HTTP awareness.
